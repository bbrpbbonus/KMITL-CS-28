...
public class NewYearTheme extends JFrame {
...
class NewYearPanel extends JPanel {
    ...
    private void drawNewYearTheme(Graphics g) {
        ...
        float x = (getWidth() - messageWidth) / 2;
        float y = (float) (getHeight() / 2);
        g2d.drawString(message, x, y);

        // วาดดอกไม้ที่เป็นสัญลักษณ์ของปีใหม่
        drawFlower(g2d, getWidth() / 1, getHeight() / 4 , 40);
        drawFlower(g2d, getWidth() / 4, getHeight() - 150, 40);
        drawFlower(g2d, getWidth() / 2, getHeight() / 4, 40);
        drawFlower(g2d, 3 * getWidth() / 4, getHeight() / 4, 40);
        drawFlower(g2d, 4 / getWidth(), getHeight() / 4, 40);
    }
